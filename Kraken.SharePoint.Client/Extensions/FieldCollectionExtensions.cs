namespace Microsoft.SharePoint.Client {
  ...
  public static class KrakenFieldCollectionExtensions {
    ...
    public static Field Add(this FieldCollection fields, FieldProperties properties, bool execute = true, ITrace trace = null) {
      ...
      Field field = fields.Add(schemaXml, execute);
    ...
    public static IEnumerable<Field> GetFieldsByGroup(this FieldCollection fields, string groupName, bool excludeBuiltInFields = false) {
      ClientContext context = (ClientContext)fields.Context;
      // Sadly CSOM does not let us use case insensitive criteria in our
      // searches, so we are forced to do this client side. BAD MICROSOFT, BAD!
      IEnumerable<Field> result = fields.GetAllFields(excludeBuiltInFields);
      return result.Where(f => groupName.Equals(f.Group, StringComparison.InvariantCultureIgnoreCase));
      /*
      IEnumerable<Field> result =
        context.LoadQuery(
          ...
          (fields.Where(
            ...
            && groupName.ToLower() == f.Group.ToLower()
    ...
    public static Field GetField(this FieldCollection fields, string identifier, FieldFindMethod findMethod = FieldFindMethod.Any) {
      ClientContext context = (ClientContext)fields.Context;
      Field field = null;
      IEnumerable<Field> result = fields.GetAllFields();
      ...
      if (findMethod == FieldFindMethod.Any) {
        ...
        result = context.LoadQuery(fields.Where(
          f => f.StaticName.ToLower() == identifier.ToLower() 
          || f.InternalName.ToLower() == identifier.ToLower()
          || f.Id.ToString().ToLower() == identifier.ToLower()
        ).IncludeKeyProperties());
        context.ExecuteQuery();
        */
        field = result.FirstOrDefault(
          f => f.StaticName.Equals(identifier, StringComparison.InvariantCultureIgnoreCase)
          || f.InternalName.Equals(identifier, StringComparison.InvariantCultureIgnoreCase)
          || f.Id.ToString().Equals(identifier, StringComparison.InvariantCultureIgnoreCase)
        );
      }
      ...
      if (field == null && findMethod == (FieldFindMethod.InternalName | FieldFindMethod.StaticName)) {
        ...
        result = context.LoadQuery(fields.Where(f =>
          f.StaticName.ToLower() == identifier.ToLower()
          || f.InternalName.ToLower() == identifier.ToLower()
        ).IncludeKeyProperties());
        context.ExecuteQuery();
        */
      ...
      if (field == null) {
        switch (findMethod) {
          case FieldFindMethod.StaticName:
            /*
            result = context.LoadQuery(fields.Where(f => f.StaticName.ToLower() == identifier.ToLower()).IncludeKeyProperties());
            context.ExecuteQuery();
            */
            field = result.FirstOrDefault(f => f.StaticName.Equals(identifier, StringComparison.InvariantCultureIgnoreCase));
            break;
          case FieldFindMethod.InternalName:
            /*
            result = context.LoadQuery(fields.Where(f => f.InternalName.ToLower() == identifier.ToLower()).IncludeKeyProperties());
            context.ExecuteQuery();
            */
            field = result.FirstOrDefault(f => f.InternalName.Equals(identifier, StringComparison.InvariantCultureIgnoreCase));
            break;
    ...
    public static void LoadKeyProperties(this FieldCollection fields, List<Field> fieldRefs = null) {
      ...
      // This was tried, but did not appear to actually work!
      //ctx.LoadQuery(fields.Where(fc => true).IncludeKeyProperties());
      ctx.Load(fields, fc => fc.Include(
        f => f.InternalName, f => f.Title, f => f.Id, f => f.StaticName, f => f.Group,
        f => f.TypeAsString, f => f.FieldTypeKind, 
        f => f.Hidden, f => f.Required, f => f.ReadOnlyField, f => f.Sealed,
