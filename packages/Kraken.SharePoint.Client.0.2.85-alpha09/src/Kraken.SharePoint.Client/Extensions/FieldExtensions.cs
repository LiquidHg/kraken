...
*/
    ...
    */
    ...
    public static List<Field> FindAny(this IEnumerable<Field> fields, IEnumerable<string> identifier) {
      // commented because this appears to return ALL fields
      /*
      List<Field> matchedFields = fields.Where(
        f => identifier.Contains(f.Title)
        || identifier.Contains(f.Id.ToString(), StringComparer.InvariantCultureIgnoreCase)
      ...
      */
      ...
      foreach (string name in identifier) {
        Field match = FindAny(fields, name);
        if (match != null)
    ...
    public static void EnsureCriticalProperties(this IEnumerable<Field> fields, ClientContext ctx) {
      foreach (Field field in fields) {
        ctx.Load(field, f => f.InternalName, f => f.Title, f => f.Id, f => f.TypeAsString);
      }
    }

    public static void EnsureCriticalProperties(this FieldCollection fields, List<Field> fieldRefs = null) {
      // ensure we have properties we'll need
      ClientContext ctx = (ClientContext)fields.Context;
      ctx.Load(fields, fc => fc.Include(
        f => f.InternalName, f => f.Title, f => f.Id, f => f.TypeAsString
      ));
      if (fieldRefs != null)
        EnsureCriticalProperties(fieldRefs, ctx);
      ctx.ExecuteQueryIfNeeded();
    }
    ...
    public static string CanonicalizeFormula(this FieldCollection fields, string formula, List<Field> fieldRefs, ITrace trace = null) {
      ...
      fields.EnsureCriticalProperties(fieldRefs);
