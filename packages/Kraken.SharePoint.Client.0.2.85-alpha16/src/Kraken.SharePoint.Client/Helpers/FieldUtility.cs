...
namespace Kraken.SharePoint.Client {
  ...
  public class FieldComparer : IEqualityComparer<Field> {
    ...
    /// <summary>
    /// True on any match for Id, Title, or InternalName
    /// </summary>
    /// <param name="x"></param>
    /// <param name="y"></param>
    /// <returns></returns>
    public bool Equals(Field x, Field y) {
      if (x == null || y == null)
        return false;
      return (x.Id == y.Id
      || x.Title.Equals(y.Title, StringComparison.InvariantCulture)
      || x.InternalName.Equals(y.InternalName, StringComparison.InvariantCultureIgnoreCase));
    }
  ...
  public class FieldUtility {
    ...
    public static bool IsFormulaFieldType(string type) {
      return (type.Equals(FieldType.Calculated.ToString(), StringComparison.InvariantCultureIgnoreCase)
      || type.Equals(FieldTypeAlias.Calculated.ToString(), StringComparison.InvariantCultureIgnoreCase));
    }
    public static bool IsUserFieldType(string type) {
      return (type.StartsWith(FieldType.User.ToString(), StringComparison.InvariantCultureIgnoreCase));
    }
    public static bool IsLookupFieldType(string type, bool userCountsAsLookup = false) {
      if (userCountsAsLookup && IsUserFieldType(type))
        return true;
    ...
    public static string GetDefaultShowField(string type) {
      return (IsUserFieldType(type)) ? "ImnName" : "Title";
      // TODO its hard to find a comprehensive list of valid options for user lookup,
      // but here are some things to try
      // ImnName, Account, UserName, Name
    }
