...
namespace Kraken.SharePoint.Client.Files {
  public abstract class BaseFileUploader : IFileTransferEngine {
    ...
    public void Upload(File targetFile, ITransferTimeEstimator estimator) {
    ...
    public void Upload(Folder targetFolder, ITransferTimeEstimator estimator) {
      if (!Params.LocalPath.IsFolder) {
        UploadSingleFile(Params.LocalPath, targetFolder, estimator);
      } else { // is a folder
        foreach (KeyValuePair<string, string> kvp in this.Map) {
          try {
            ...
            if (f.IsFolder) {
              ...
              if (folder == null)
                throw new System.IO.FileNotFoundException(string.Format("Required parent folder '{0}' could not be read and may not exist.", parentFolderUrl));
              UploadSingleFile(f, folder, estimator);
            }
          } catch (Exception ex) {
    ...
    private void UploadSingleFile(LocalFileOrFolder f, Folder targetFolder, ITransferTimeEstimator estimator) {
      if (f.IsFolder)
        return;
      // TODO event to update progress writer
      Trace.TraceVerbose("Uploading file '{0}'...", f.FileName);
      Trace.TraceVerbose("Uploading type: {0}", this.GetType().Name);
      File newFile;
      using (var stopWatch = new StopWatchTrace(Trace)) {
        newFile = DoUpload(f, targetFolder, estimator);
        estimator.ActualTime = stopWatch.Stop();
      }
    ...
    public void Download(Folder folder, ITransferTimeEstimator estimator) {
      _rootFolder = folder;
      DownloadSingleFolder(Params.LocalPath, folder, estimator);
    }

    public void Download(File file, ITransferTimeEstimator estimator) {
      _rootFolder = file.GetParentFolder();
      DownloadSingleFile(Params.LocalPath, file, estimator);
    }

    private void DownloadSingleFolder(LocalFileOrFolder f, Folder folder, ITransferTimeEstimator estimator) {
      ...
      foreach (File file in folder.Files) {
        ...
        DownloadSingleFile(subf, file, estimator);
      }
      if (Params.RecurseSubfolders) {
        foreach (Folder subFolder in folder.Folders) {
          string local = this.Map.ConvertToLocalPath(subFolder, _rootFolder);
          LocalFileOrFolder subf = new LocalFileOrFolder(local);
          DownloadSingleFolder(subf, folder, estimator);
        }
      }
    }

    private void DownloadSingleFile(LocalFileOrFolder f, File file, ITransferTimeEstimator estimator) {
      Trace.TraceVerbose("Downloading file '{0}'...", file.Name);
      using (var stopWatch = new StopWatchTrace(Trace)) {
        DoDownload(f, file, estimator);
        estimator.ActualTime = stopWatch.Stop();
      }
    ...
    protected abstract File DoUpload(LocalFileOrFolder f, Folder targetFolder, ITransferTimeEstimator estimator);

    protected abstract void DoDownload(LocalFileOrFolder f, File file, ITransferTimeEstimator estimator);
    ...
    */
    ...
    protected void DownloadFromOpenBinaryDirect(LocalFileOrFolder f, File file, ITransferTimeEstimator estimator) {
      if (f.IsFolder)
