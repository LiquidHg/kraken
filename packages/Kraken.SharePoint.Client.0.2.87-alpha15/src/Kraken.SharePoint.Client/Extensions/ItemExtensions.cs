  ...
  public static class KrakenItemExtensions {
    ...
    public static string GetNameOrTitle(this ListItem item, Hashtable fieldValues, UpdateItemOptions options = null, ITrace trace = null) {
      ...
      if (fieldValues != null && fieldValues.ContainsKey(options.TitleInternalFieldName)) {
        ...
        // should have already been previously loaded but check anyway
        item.EnsureProperty(i => i[options.TitleInternalFieldName]);
    ...
    public static void EnsureFields(this ListItem item, IEnumerable<string> fieldNames, ITrace trace) {
      ...
      foreach (string fieldName in fieldNames) {
        ctx.LoadIfRequired(item, trace, false, i => i[fieldName]);
      }
    ...
    /// fields that are empty when OverwriteEmpty option is false,
    /// and fields where there is already data in the item when
    /// OverwriteExistingData is false.
    /// </summary>
    /// <param name="fieldValues"></param>
    /// <param name="item"></param>
    /// <param name="options"></param>
    /// <param name="trace"></param>
    /// <returns></returns>
    private static Hashtable CheckSkipFields(Hashtable fieldValues, ListItem item = null, UpdateItemOptions options = null, ITrace trace = null) {
      if (fieldValues == null)
      ...
      if (!options.OverwriteExistingData) {
        ...
        if (item == null)
          trace.TraceWarning("OverwriteExistingData option is false, but can't check item for existing data because it was not provided.");
        else
          item.EnsureFields(fieldValues.KeysAsArray(), trace);
      }
      Hashtable ht2 = new Hashtable();
      foreach (string fieldName in fieldValues.Keys) {
        object newVal = fieldValues[fieldName];

        if (newVal != null && newVal.ToString() == FIELD_TOKEN_KEEP_CURRENT_VALUE) {
        ...
        if (!options.OverwriteEmpty
          && newVal == null
          || ( // technically, I think we don't have to check for newVal != null but it doesn't hurt
            newVal != null && string.IsNullOrWhiteSpace(newVal.ToString())
          )
        ) {
          if (!options.SupressSkippedFieldWarnings)
        ...
        if (item != null && !options.OverwriteExistingData) {
          object itemVal = item[fieldName];
          if (itemVal != null && !string.IsNullOrWhiteSpace(item[fieldName].ToString())) {
            if (!options.SupressSkippedFieldWarnings)
    ...
    public static UpdateItemResult UpdateItem(this ListItem item, Hashtable fieldValues, UpdateItemOptions options = null, ITrace trace = null) { // , WebContextManager contextManager = null
      ...
      fieldValues = CheckSkipFields(fieldValues, item, options, trace);
      if (fieldValues.Count == 0)
        return UpdateItemResult.UpdateOK;
      ...
      if (doExec) {
        ...
        trace.Trace(TraceLevel.Verbose, (options.UpdateFrequency == ItemUpdateFrequency.OncePerItem) ? "Updating metadata fields in item..." : "Updating items...");
        context.ExecuteQuery();
