...
#region Custom Properties
    ...
    public static void Flags(this List list, ulong value, WebContextManager contextManager, XElement listNode = null) {
       ...
       */
        ...
        kcloud.SharePointConnection conn = contextManager.CreateLegacyConnection(true, false);
        conn.ListsManager.UpdateListSchema(list.Id, listNode, null, false);
    ...
    public static void SetNavigateForFormsPages(this List list, bool value, WebContextManager cm, ITrace trace = null) { // , WebContextManager contextManager
      ...
      if (trace == null) trace = NullTrace.Default;
      bool? current = list.NavigateForFormsPages();
      ...
      XElement listSchema = connection.ListsManager.GetListSchema(list.Id);
      // make change to XML
      string propName = "NavigateForFormsPages";
      if (current.HasValue) {
        // node exists, change it
        XAttribute navigateAttrib = listSchema.Attributes().Where(x => x.Name == propName).FirstOrDefault();
        navigateAttrib.SetValue(value.ToString().ToUpper());
      } else {
        // insert a new node
        listSchema.Add(new XAttribute(propName, value.ToString().ToUpper()));
      }
      // TODO should we also set flags here?
      XElement result = connection.ListsManager.UpdateListSchema(list.Id, listSchema, null, false);
