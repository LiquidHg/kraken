...
namespace Kraken.SharePoint.Logging.Unsafe {
  ...
  [Flags]
  enum TraceFlags {
    None = 0, TRACE_FLAG_START = 1, TRACE_FLAG_END = 2, TRACE_FLAG_MIDDLE = 3,
    // notice middle==start+end, allows use as a bitmask 
    TRACE_FLAG_FLUSH = 8,
    // ask trace manager to flush all file buffers 
  }
