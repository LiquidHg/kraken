namespace Kraken.SharePoint.Smtp {
  ...
  public class MailMessageExtended : System.Net.Mail.MailMessage {
    ...
    private KrakenLoggingService uls;
    ...
    /// <summary>
    /// When true, logs all sucessful email sends, otherwise logs only failures.
    ...
    public bool VerboseLogging {
      ...
      set {
        ...
        if (uls != null)
          uls.DefaultCategory = verboseLogging ? LoggingCategories.CustomVerbose : LoggingCategories.CustomMedium;
      }
    }
    ...
    private string Merge(string text) {
      if (this.MergeFieldDictionary != null && this.MergeFieldDictionary.Count > 0) {
        text = StringTools.MergeFields(this.MergeFieldDictionary, text);
      }
      if (this.MergeFieldRow != null) {
        text = StringTools.MergeFields(this.MergeFieldRow, text);
      }
      return text;
    }
    ...
    private void AsyncSendCompleted(object source, AsyncCompletedEventArgs args) {
      string statusMessage = args.UserState.ToString();
      // log successes
      uls.Write(statusMessage);
      if (args.Error != null) {
        WrapSendException(statusMessage, args.Error);
